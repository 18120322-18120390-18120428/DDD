...
export class Booking {
  ...
  static cancelLesson = async (cancelLesson) => {
    try {
      ...
      if (
        bookedLesson == null ||
        bookedLesson.tutee._id != cancelLesson.tutee
      ) {
  ...
  static getBookingByTutee = async (getBookedLesson) => {
    try {
      const bookingCollections = await new BookingDao().findByCondition(
        getBookedLesson
      );

      return bookingCollections.map((booking) =>
        Booking.mappingFromBookingRepository(booking)
      );
    } catch (err) {
      // Error handling logic should go here
      throw new Error(`Get failed: ${err.message}`);
